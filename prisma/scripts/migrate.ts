...
for (const legacyModule of legacyModules) {
    ...
    if (!userId)
        throw new Error(`Unknown legacy user ID ${legacyModule.user_id} for module ${legacyModule.name}`);
    ...
    for (const legacyRelease of legacyReleases) {
        ...
        if (legacyRelease.verification_token !== null)
            bar.log(`Warning: Release ${uuid} of module ${legacyModule.id} has a pending verification token\n`);
    }
