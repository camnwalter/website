...
export default function SearchBar({ sx = [] }: Props) {
  ...
  useEffect(() => {
    const initialChips = Object.entries(router.query)
      .filter(([name]) => chipOptions.find(o => o[0] === name))
      .map(([name, value]) => `${name}:${value}`);
    const initialValue =
      (Array.isArray(router.query["q"]) ? router.query["q"].at(-1) : router.query["q"]) ?? "";
  ...
  const handleSubmit = (): void => {
    ...
    router.replace({ pathname: "/modules", query: queryParams });
  };
